...
static void v2rinit_pwm(void){
	...
	pwm[i]->pcr = 0;
        pwm[i]->cfg = 0;
        pwm[i]->start = 0;
        pwm[i]->repeat = 0xFFFFFFFF;
        pwm[i]->period = 0xFF*3;
        pwm[i]->ph1d = 0xFF*3;
     }
};
static void v2rstart_pwm(unsigned int number, unsigned int duty){
     printk("Starting pwm %d, duty %d\r\n", number, duty);
     if (number > 3){
          printk("Error in setting pwm number\r\n");
          return;
     }
     if (duty > 0xFF) duty = 0xFF*3;
     pwm[number]->pcr   = 0x01;
     pwm[number]->cfg   = 0x12;//Continious mode
     pwm[number]->ph1d  = duty*3;
     pwm[number]->start = 1;
};
...
static char** split(const char* str, char delimiter){
    ...
    result = kmalloc((nBites+1)*sizeof(char*),GFP_KERNEL);
    for (nIndex = 0; nIndex <= input_len; nIndex++){
...
static int is_equal_string(const char* str1, const char* str2){
    int len = strlen(str1);
    int i = 0;
    if (len != strlen(str2)){ printk("IS EQUAL STRING LEN ERROR\r\n"); return FALSE;}
    for (i = 0; i < len; i++){
       if (str1[i] != str2[i]) { printk("IS EQUAL STRING COMPARE ERROR %s %s %d\r\n", str1, str2, i); return FALSE;}
    }
    return TRUE;
}
...
static int starts_with(const char* str1, const char* ref){
    ...
    int len = strlen(str1);
    const char* substr = 0;
    if (len <= strlen(ref)) {printk("starts with len error\r\n"); return NA;}
    len = strlen(ref);
    for (i = 0; i < len; i++){
       if (str1[i] != ref[i]) {printk("starts with not equal error %d\r\n", i); return NA;}   
    }
...
static void process_pwm(char** data, unsigned int ordinal){
     int duty = -1;
     if (!data[2]) {printk("Error pwm parameter\r\n"); return;}; 
...
static void process_set(char** data){
      ...
      if (data[1]){//Any parse result present
          ...
          printk("Ordinal = %d\r\n", ordinal);
          if (ordinal > 0 && ordinal <= TOTAL_PINS) { printk("Processing CON%d\r\n", ordinal); process_con(data, ordinal); return; }
          ordinal = starts_with(data[1], "pwm");//This is pwm hardware
          if (ordinal >= 0 && ordinal < 4) { printk("Processing PWM%d\r\n", ordinal); process_pwm(data, ordinal); return; }
      }
